...
def get_args():
    parser = argparse.ArgumentParser(description="Batch quality prediction using CLIP-FIQA")
    parser.add_argument(
        "--folder", type=str, required=True,
        help="Path to the folder containing images"
    )
    parser.add_argument(
        "--output", type=str, default="quality_scores.txt",
        help="Path to the output text file"
    )
    parser.add_argument(
        "--clip_model", type=str, default="./weights/RN50.pt",
        help="Path to the CLIP model weights"
    )
    parser.add_argument(
        "--clip_weights", type=str, default="./weights/CLIB-FIQA_R50.pth",
        help="Path to the fine-tuned CLIP-FIQA weights"
    )
...
if __name__ == "__main__":
    ...
    all_images = [os.path.join(args.folder, f)
    ...
    with open(args.output, 'w') as fout:
        for image_path in all_images:
            ...
            scores.append(quality_preds)
            fout.write(f"{os.path.basename(image_path)}: {quality_preds:.6f}\n")
        
        avg_score = np.mean(scores) if scores else 0.0
        fout.write(f"Average quality score: {avg_score:.6f}\n")
        print(f"Average quality score: {avg_score:.6f}")
