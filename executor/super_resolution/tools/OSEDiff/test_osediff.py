...
if __name__ == "__main__":
    ...
    if os.path.isdir(args.input_image):
        image_names = natsorted(glob.glob(f'{args.input_image}/*.[pPbBjJ][nNmMgGpP]*') + glob.glob(f'{args.input_image}/*.raw'))
    else:
    ...
    for image_name in image_names:
        ...
        # if (os.path.splitext(image_name)[1].lower() == '.raw'):
        #     import rawpy
        #     # with rawpy.imread(image_name) as raw:
        #     #     rgb = raw.postprocess()
        ...
        if input_image.size[0] % 8 != 0 or input_image.size[1] % 8 != 0:
        ...
        with torch.no_grad():
            ...
            if resize_flag:
                ...
                # print(f"resize {bname} to {int(args.upscale*ori_width)}x{int(args.upscale*ori_height)}")
                # print(f"output_pil size: {output_pil.size}")
