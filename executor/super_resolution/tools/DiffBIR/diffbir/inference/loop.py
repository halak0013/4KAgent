...
class InferenceLoop:
    ...
    def __init__(self, args: Namespace) -> "InferenceLoop":
        ...
        # get path
        self.config_dir = Path(__file__).resolve().parent.parent.parent / "configs" / "inference"
    ...
    def load_cldm(self) -> None:
        cldm_config = self.config_dir / "cldm.yaml"
        self.cldm: ControlLDM = instantiate_from_config(OmegaConf.load(cldm_config))
        ...
        if self.args.version in ["v1", "v2"]:
            diffusion_config = self.config_dir / "diffusion.yaml"
        else:
            diffusion_config = self.config_dir / "diffusion_v2.1.yaml"
        self.diffusion: Diffusion = instantiate_from_config(OmegaConf.load(diffusion_config))
