...
class Diffusion(object):
    ...
    def sample(self):
        ...
        if self.config.model.class_cond:
            ckpt = os.path.join("ckpts", 'imagenet/%dx%d_diffusion.pt' % (
            ...
            ckpt = os.path.join(f"{script_dir}/ckpts", "imagenet/256x256_diffusion_uncond.pt")
            if not os.path.exists(ckpt):
                download(
    ...
    def FourierDiff(self, model,cls_fn):
        ...
        for input_y, name in pbar:
            ...
            tvu.save_image(
                x[0], os.path.join(self.args.output_dir, f"{name}")
            )
