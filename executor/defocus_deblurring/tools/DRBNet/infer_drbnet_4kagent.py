...
for i, filename in enumerate(input_c_file_path_list):
    # Read Image
    C = read_image(input_c_file_path_list[i], 255.0) # here to [0,255]
    _, ori_h, ori_w, _ = C.shape
    # C = pad_image(read_image(input_c_file_path_list[i], 255.0))*2-1
    C = pad_image(C)*2-1 
    C = torch.FloatTensor(C.transpose(0, 3, 1, 2).copy()).cuda()
    filename = os.path.split(filename)[-1]

    if opt.net_mode == 'dual':
        R_in = read_image(input_r_file_path_list[i], 255.0) # here to [0,255]
        _, _, ori_h, ori_w = R_in.shape
        R,L = pad_image(read_image(input_r_file_path_list[i], 255.0))*2-1, pad_image(read_image(input_l_file_path_list[i], 255.0))*2-1
    ...
    if gt_file_path_list is not None:
        GT_in = read_image(gt_file_path_list[i], 255.0) # here to [0,255]
        # _, _, ori_h, ori_w = GT_in.shape
        # GT = pad_image(read_image(gt_file_path_list[i], 255.0))  # here to [0,1]
    ...
    with torch.no_grad():
        ...
        if opt.net_mode == 'single':
            ...
            b, c, h, w = C.shape
            if opt.tile is None or max(h, w) <= opt.tile:
                output = network(C)
                ...
                E = torch.zeros(b, c, h, w).type_as(C)
                W = torch.zeros_like(E)

                for h_idx in h_idx_list:
                    for w_idx in w_idx_list:
                        in_patch = C[..., h_idx:h_idx+tile, w_idx:w_idx+tile]
                        out_patch = network(in_patch)
    ...
    # crop back to original size
    output_cpu = output_cpu[0:ori_h, 0:ori_w, :]
