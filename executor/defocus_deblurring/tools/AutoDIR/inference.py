...
artifact_category = ['noise', 'blur', 'rain', 'underexposure', 'haze', 'low resolution', 'raindrop', 'no']
...
def main():
    ...
    parser.add_argument("--ckpt",default="checkpoints/autodir.ckpt",type=str)
    ...
    # input = args.input
    input = glob(os.path.join(args.input_dir, "*"))[0]
    if args.decoder_consistency:
        decoder_consistency = ConsistencyDecoder(device="cuda")

    name = os.path.basename(input).split('.')[0]
    print(f"current image name: {name}")
